...
from logic import (
    ...
    selected_reference_quality_issues,
        ...
        rows.append(
            {
                ...
                "reference_quality": "; ".join(paper.get("reference_quality_issues") or selected_reference_quality_issues(paper)),
                "rank": index + 1,
...
def paper_selection_editor(papers: list[dict], key: str) -> pd.DataFrame:
    return st.data_editor(
        ...
        column_config={
            ...
            "reference_quality": st.column_config.TextColumn(
                "Reference quality",
                width="medium",
                help="Blank means citable metadata looks usable. Warnings mean the source can still be used for search/mining but should not be cited directly.",
            ),
                ...
                for paper, note in thesis_notes:
                    ...
                    if note.get("objective_matched_rol_extracts"):
                        ...
                        for extract in note.get("objective_matched_rol_extracts", [])[:8]:
                            st.write(extract)
                    if note.get("objective_matched_rol_full_block_coverage"):
                        st.markdown("**Full matched RoL block coverage**")
                        for coverage in note.get("objective_matched_rol_full_block_coverage", [])[:8]:
                            st.write(coverage)
